...
public class KalmanLocationManager {
    ...
    /**
     * Register for {@link android.location.Location Location} estimates using the given
     * LocationListener callback.
     *
     * @param useProvider             Specifies which of the native location providers to use, or
     *                                a combination of them.
     * @param minTimeFilter           Minimum time interval between location estimates, in
     *                                milliseconds.
     *                                Indicates the frequency of predictions to be calculated by
     *                                the filter,
     *                                thus the frequency of callbacks to be received by the given
     *                                location listener.
     * @param minTimeGpsProvider      Minimum time interval between GPS readings, in milliseconds.
     *                                If {@link UseProvider#NET UseProvider.NET} was set, this
     *                                value is ignored.
     * @param minTimeNetProvider      Minimum time interval between Network readings, in
     *                                milliseconds.
     *                                If {@link UseProvider#GPS UseProvider.GPS} was set, this
     *                                value is ignored.
     * @param listener                A
     *                                {@link android.location.LocationListener LocationListener}
     *                                whose
     *                                {@link android.location.LocationListener#onLocationChanged(android.location.Location) onLocationChanged(Location)}
     *                                method will be called for each location estimate produced
     *                                by the filter. It will also receive
     *                                the status updates from the native providers.
     * @param forwardProviderReadings Also forward location readings from the native providers to
     *                                the given listener.
     *                                Note that <i>status</i> updates will always be forwarded.
     */
    public void requestLocationUpdates(UseProvider useProvider, long minTimeFilter, long
            minTimeGpsProvider, long minTimeNetProvider, LocationListener listener, boolean
            forwardProviderReadings) {
        // Validate arguments
        if (useProvider == null) throw new IllegalArgumentException("useProvider can't be null");
        ...
        if (mListener2Thread.containsKey(listener)) {
            ...
            Log.d(TAG, "Requested location updates with a listener that is already in use. " +
                    "Removing.");
            removeUpdates(listener);
        }

        LooperThread looperThread = new LooperThread(mContext, useProvider, minTimeFilter,
                minTimeGpsProvider, minTimeNetProvider, listener, forwardProviderReadings);
    ...
    public void removeUpdates(LocationListener listener) {
        ...
        if (looperThread == null) {
            ...
            Log.d(TAG, "Did not remove updates for given LocationListener. Wasn't registered in "
                    + "this instance.");
            return;
        }
